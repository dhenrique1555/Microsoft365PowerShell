...
ForEach($Device in $Get_All_Devices)
	{	
		...
		$Device_Owner_Obj | Add-Member NoteProperty -Name "Is Device enabled ?" -Value $Device_Account_Status -force		
		...
		If($Count_Device_Owners -eq 0)
			{
				...
				$Device_Owner_Obj | Add-Member NoteProperty -Name "Device Owner UPN" -Value "No owner" -force		
		...
		ElseIf($Count_Device_Owners -gt 1)
			{
				...
				$Device_Owner_Obj | Add-Member NoteProperty -Name "Device Owner UPN" -Value $Owners_Mail -force
		...
		Else
			{
				...
				$Device_Owner_Obj | Add-Member NoteProperty -Name "Device Owner UPN" -Value $Owner_Mail -force		
...
$Devices_report	| export-csv "C:\temp\list_devices_ownerteste.csv" -notype -delimiter ";" -encoding utf8
